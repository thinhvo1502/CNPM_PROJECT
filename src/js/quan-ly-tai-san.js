var tableData = [];

var table = new Tabulator("#tenant-asset-table",{
    data: tableData,
    layout: "fitColumns",
    placeholder: "<div><img src = 'data:image/png;base64,' alt = 'No Data Image'><br>Không tìm thấy dữ liệu! </div>",
    columns: [
        {title: "Tên tài sản", field: "name", hozAlign: "center"},
        {title: "Giá trị nhập vào", field: "importValue", hozAlign: "right"},
        {title: "Giá trị tài sản", field: "assetValue", hozAlign: "right"},
        {title: "Tổng số lượng", field: "totalQuantity", hozAlign: "center"},
        {title: "Tổng số đang sử dụng", field: "usedQuantity", hozAlign: "center"},
        {title: "Tổng số còn dư", field: "remainingQuantity", hozAlign: "center"},
        {title: "Đơn vị", field: "unit", hozAlign: "center"},
        {title: "Tình trạng", field: "status", hozAlign: "center"},
    ]
    
})